...
public class DiscoverQuizStartActivity extends AppCompatActivity {
   ...
   String quizTitle;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_discover_quiz_start);



        countDownTxt = findViewById(R.id.discoverQuizCountdownTextView);
        questionTxt = findViewById(R.id.discoverQUizQuestionTxt);
        questionNumberTxt = findViewById(R.id.discoverQuizQuetionCount);
        optionFirstTxt = findViewById(R.id.discoverOptionFirstTxt);
        optionSecondTxt = findViewById(R.id.discoverOptionSecTxt);
        optionThirdTxt = findViewById(R.id.discoverOptionThirdTxt);
        optionFourthTxt = findViewById(R.id.discoverOptionFourthTxt);
        previousImg = findViewById(R.id.discover_quiz_previous_btn);
        nextImg = findViewById(R.id.discover_quiz_next_btn);
        nextBtn = findViewById(R.id.discover_next_quiz_button);
        submitBtn = findViewById(R.id.discover_submit_quiz_button);
        optionFirstLy = findViewById(R.id.discoverOptionFirstLy);
        optionSecondLy = findViewById(R.id.discoverOptionSecondLy);
        optionThreeLy = findViewById(R.id.discoverOptionThreeLy);
        optionFourthLy = findViewById(R.id.discoverOptionFourthLy);

        Toolbar toolbar = findViewById(R.id.toolbarDiscoverQuiz);
        TextView toolbarTxt = toolbar.findViewById(R.id.discoverQuizStartName);
    ...
    private void fetchQuestionsFromFirebase() {
        Intent intent = getIntent();
        String  quizTopic = intent.getStringExtra("quizTitle");


        DatabaseReference quizRef = FirebaseDatabase.getInstance().getReference("DiscoverQuiz").child(quizTopic).child("questions");
    ...
    private void startTimer() {
        countDownTimer = new CountDownTimer(timeLeftInMillis, 1000) {
            ...
            public void onFinish() {
                ...
                Toast.makeText(DiscoverQuizStartActivity.this, "Time's up!", Toast.LENGTH_SHORT).show();
            }
        }.start();
    ...
    private void updateCountDownText() {
        ...
        TextView countdownTextView = findViewById(R.id.discoverQuizCountdownTextView);
    ...
    private void applySelectedOptionStyle(int selectedOptionId) {
        if (selectedOptionId == R.id.discoverOptionFirstLy) {
            selectOption(optionFirstLy, optionFirstTxt);
        } else if (selectedOptionId==R.id.discoverOptionSecondLy) {
            selectOption(optionSecondLy, optionSecondTxt);
        } else if (selectedOptionId==R.id.discoverOptionThreeLy) {
            selectOption(optionThreeLy, optionThirdTxt);
        }else {
            selectOption(optionFourthLy, optionFourthTxt);
        }
    ...
    private void uploadSelectedOptionsToFirebase() {
        ...
        DatabaseReference userRef = FirebaseDatabase.getInstance().getReference("discoverQuizUserAnswer").child(quizTitle).child(userId);
        int score = 0;
       ...
       uploadScoreToFirebase(score);
        Intent intent = new Intent(DiscoverQuizStartActivity.this, QuizeScoreActivity.class);
        intent.putExtra("scoreOfDiscoverQuiz", score);
    ...
    private void uploadScoreToFirebase(Integer scoreOfUser){
        ...
        if (currentUser != null) {
            String userId = currentUser.getUid();
            DatabaseReference userScoreRef = FirebaseDatabase.getInstance().getReference("discoverUserScore").child(quizTitle).child(userId);
    ...
    public void onBackPressed() {
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                // If the user confirms, go back to the QuizOfTheWeekActivity

                DiscoverQuizStartActivity.super.onBackPressed();
            }
        });
        builder.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
            @Override
